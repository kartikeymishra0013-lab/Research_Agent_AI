...
Wires together: config loading → service initialization → ingestion →
agent processing → storage → result aggregation.
"""
...
class PipelineOrchestrator:
    ...
    def process_file(self, file_path: str) -> dict:
        ...
        return self._process_source(file_path)

    def process_url(self, url: str) -> dict:
        """Process a document from a URL."""
        return self._process_source(url)

    def process_directory(self, dir_path: str, glob_pattern: str = "**/*") -> dict:
        """
        ...
        results = []
        for f in files:
            result = self._process_source(str(f))
    ...
    def _process_source(self, source: str) -> dict:
    ...
    def _persist_results(self, result: dict, source: str):
        ...
        if "generate_summary" in tool_results:
            ...
            self.json_store.save_summary_report(summary, source, metadata=result.get("metadata"))
    ...
    def _load_config(config_path: Path, overrides: dict) -> dict:
        ...
        # Apply overrides (nested merge)
        _deep_merge(config, overrides)

        # Resolve env var placeholders like ${OPENAI_API_KEY}
